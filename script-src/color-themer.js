...
const generateFavicon = () => {
  ...
  const color1 = rootStyle.getPropertyValue("--color-accent-a");
  // const color1 = currentThemes[currentColorMode].accentA;
  const color2 = rootStyle.getPropertyValue("--color-accent-b");
...
let tickAnimationCounter = 0;
const tickAnimationMax = 5;
const tickAnimations = () => {
  tickAnimationCounter++;
  if (tickAnimationCounter === tickAnimationMax) {
...
// get the current color mode from the window
...
/**
 * This function initializes the CSS variables in the root style element so that transitions will work. It sets them to be the same as the current theme colors.
 */
const initializeColors = () => {
  ...
  let curColorSet;
  if (currentColorMode === "dark") {
    curColorSet = currentThemes.dark;
  } else {
    curColorSet = currentThemes.light;
  }
  //apply colors to page
  Object.values(curColorSet).forEach((item) => {
    console.log(item);
    const colorRGB = gsap.utils.splitColor(
      `hsl(${item.hue}deg ${item.sat}% ${item.lum}%)`
    );
    const cssVarValue = `rgb(${colorRGB.join(",")})`;
    rootStyle.setProperty(`--${item.cssName}`, cssVarValue);
  });
};
initializeColors();
